package org.mcsmtp.blescanner.wayfinder.filter;
...
public class RssiFilterPipeline {
    ...
    // 중앙값만 적용
    public double filterMedianOnly(double measurement) {
        return applyMedianFilter(measurement);
    }

    // 칼만만 적용 (중앙값 안 거치고 원본에 바로)
    public double filterKalmanOnly(double measurement) {
        return applyKalmanFilter(measurement);
    }

    // 히스테리시스 상태를 숫자로 변환해서 그래프에 그릴 수 있게 함
    // ARRIVED면 진입 임계값, DEPARTED면 이탈 임계값으로 표시 (계단 모양 선이 됨)
    public double filterHysteresisOnly(double measurement) {
        double medianFiltered = applyMedianFilter(measurement);
        double kalmanFiltered = applyKalmanFilter(medianFiltered);
        applyHysteresis(kalmanFiltered);
        return state == BeaconState.ARRIVED ? entryThreshold : exitThreshold;
    }
